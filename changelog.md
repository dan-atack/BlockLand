...
### 15. Ensure that Items are regenerated by the Engine in the event of the game having to restart.

### 16. Ensure that Items are re-rendered after the game's Menu is closed.

### 17. Analyze what should be done (if anything) to ensure that new game content can be added more easily without having to individually perform these last 2 steps (are we talking unit tests? a 'new features' checklist describing where to put things in the Engine?)

### 18. Modify the wetlands biome to serve as an introduction to the Items, and have it include getting a very long-lasting dose of the steroids and they're introduced at a moment where you just need to make a big jump to keep going in that moment (the player must take drugs in order to complete a really hard jump which would be impossible without chemical enhancement). Keep the heart on the opposite side of the first baddie (the one on the block over the laval) Cumulatively, this will serve as an introduction to the Items and Hitpoint features. Lastly, add some XP Items to both ends of the map when the 'touch both walls' objective is met. That way, at the end of the course you can have the Player get some experience, which he can spend in our next big feature, which is coming very soon now...

### 16. Push to production and close the branch!
